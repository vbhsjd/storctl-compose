...
这篇文档给普通使用者看。管理员会提前准备好 `storctl-profiles.json`，用户只需要填写机器登录信息，然后执行 `apply`、`report`。`apply` 会自动上传工具和配置；`copy` 只在排查上传问题时单独使用。
...
```bash
./storctl-compose apply --limit 141.61.50.185
./storctl-compose apply --limit 141.61.50.185,141.61.50.141
```
...
上传或执行过程中 SSH/SFTP 断开。可以单台重跑：

```bash
./storctl-compose apply --limit 目标IP --timeout 60m
```

`networkmanager_down`

目标机器 NetworkManager 没运行，且自动启动失败。需要在目标机上检查：
